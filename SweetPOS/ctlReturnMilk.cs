...
namespace SweetPOS
{
    public partial class ctlReturnMilk : UserControl
    {
        ReturnMilkManager _milkIsssueManager = new ReturnMilkManager();
        bool _allowEdit = true;

        public ctlReturnMilk()
        {
            InitializeComponent();
        }
        ...
        {
            frmReturnMilk frm = new frmReturnMilk();
            if (frm.ShowDialog() == DialogResult.OK)
            {
                SearchLineManMilkReturn();
        ...
        public bool SearchLineManMilkReturn()
        {
        ...
        bool DeleteItem()
        {
            if (grdResult.SelectedRows.Count > 0)
            {
                try
                {
                        ...
                        {
                            _milkIsssueManager.DeleteMilkReturn(Convert.ToInt32(grdResult.SelectedRows[0].Cells["ID"].Value));
                            SearchLineManMilkReturn();
                            return true;
                        }
