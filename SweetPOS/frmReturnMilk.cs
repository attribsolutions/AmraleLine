...
namespace SweetPOS
{
    public partial class frmReturnMilk : Form
    {
        ReturnMilkManager _MilkIssueManager = new ReturnMilkManager();
        ...
        bool ShowItemsforMilkReturnEntry()
        {
            BindingList<MilkIssueItemInfo> MilkIssueItems = new BindingList<MilkIssueItemInfo>();
            try
            {
                MilkIssueInfo milkIssuelinemanInfo = new MilkIssueInfo();
                milkIssuelinemanInfo.ID = Convert.ToInt32(cboLineman.SelectedValue);
                milkIssuelinemanInfo.MilkIssueDate = Convert.ToDateTime(dtMilkIssueDate.Value).Date;


                MilkIssueItems = _MilkIssueManager.ShowItemsforMilkReturnEntry(milkIssuelinemanInfo);
                ...
                foreach (MilkIssueItemInfo orderItem in MilkIssueItems)
                {
                    ...
                    grdResult.Rows[row].Cells["Quantity"].Value = orderItem.Quantity;
                    grdResult.Rows[row].Cells["ReturnQuantity"].Value = orderItem.ReturnQuantity;
        ...
        bool Save()
        {
            ...
            if (_viewMode == EnumClass.FormViewMode.Addmode)
            {
                try
                {
                    _newRecordID = Convert.ToInt32(_MilkIssueManager.ReturnMilkByLineman(MilkIssue, MilkIssueItems));
                    _newRecordAdded = true;
                    return true;
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Error in saving  Milk Return." + Environment.NewLine + Environment.NewLine + "(" + ex.Message + ")", Program.MESSAGEBOXTITLE, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return false;
                }
            }
            return false;
        ...
        LinemanInfo SetValue(out BindingList<MilkIssueItemInfo> MilkIssueItemss)
        {
            ...
            foreach (DataGridViewRow row in grdResult.Rows)
            {
                ...
                if (Convert.ToDecimal(row.Cells["Quantity"].Value) > 0)
                {
                    ...
                    items.ReturnQuantity = Convert.ToDecimal(row.Cells["ReturnQuantity"].Value);
                }
        ...
        private void frmReturnMilk_Load(object sender, EventArgs e)
        {
            FillLineMans();
        }
    }
